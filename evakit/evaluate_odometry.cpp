...
vector<float> trajectoryDistances (vector<Matrix> &poses) {
  vector<float> dist;
  dist.push_back(0);
  for (int32_t i=1; i<poses.size(); i++) { //逐2帧
    Matrix P1 = poses[i-1];
    Matrix P2 = poses[i];
    float dx = P1.val[0][3]-P2.val[0][3]; //每两帧的平移之差
    float dy = P1.val[1][3]-P2.val[1][3];
    float dz = P1.val[2][3]-P2.val[2][3];
    dist.push_back(dist[i-1]+sqrt(dx*dx+dy*dy+dz*dz)); //当前帧相较初始帧的位移，dist(0)=0
  }
  return dist;
}

//若第i帧到first_frame的位移超过len，返回i；否则返回-1
int32_t lastFrameFromSegmentLength(vector<float> &dist,int32_t first_frame,float len) {
  for (int32_t i=first_frame; i<dist.size(); i++) //dist size是总帧数
    if (dist[i]>dist[first_frame]+len)
      return i; 
  return -1;
}

//按照论文，应该是求rotation angle  也就是旋转矩阵R->旋转向量的变换
inline float rotationError(Matrix &pose_error) {
  float a = pose_error.val[0][0];
  float b = pose_error.val[1][1];
  float c = pose_error.val[2][2]; //pose_error矩阵中R部分的对角线
  float d = 0.5*(a+b+c-1.0); //tr(R)=a+b+c，根据《14讲》（3.16），/theta=arcos[(tr(R)-1)/2]
  return acos(max(min(d,1.0f),-1.0f)); //why not 直接acos(d) 避免超出（-1,1）范围吧
}

//按照论文，用该就矩阵的2范数啊，为何只考虑平移部分的范数?
inline float translationError(Matrix &pose_error) {
  ...
  return sqrt(dx*dx+dy*dy+dz*dz); //pose_error矩阵中平移部分的距离 
}
...
vector<errors> calcSequenceErrors (vector<Matrix> &poses_gt,vector<Matrix> &poses_result) {
  ...
  // parameters
  int32_t step_size = 10; // every second  数据本是10Hz
  
  // pre-compute distances (from ground truth as reference)
  vector<float> dist = trajectoryDistances(poses_gt); //每一帧相比初始帧的位移
 
  // for all start positions do
  for (int32_t first_frame=0; first_frame<poses_gt.size(); first_frame+=step_size) { //间隔10帧 数据是10Hz,这样就是每隔1秒作为起点
  
    // for all segment lengths do
    for (int32_t i=0; i<num_lengths; i++) { //num_lengths=8
    
      // current length
      float len =lengths[i]; //(i+1)*100
      ...
      // compute rotational and translational errors  VSLAM14讲 P60 
      Matrix pose_delta_gt     = Matrix::inv(poses_gt[first_frame])*poses_gt[last_frame]; //gt 位姿 :first_frame的位姿 的 逆* last_frame 的位姿  就是last frame相对于first frame的位姿
      Matrix pose_delta_result = Matrix::inv(poses_result[first_frame])*poses_result[last_frame]; //预测位姿：first_frame的位姿 的 逆* last_frame 的位姿的逆 就是last frame相对于first frame的位姿
      Matrix pose_error        = Matrix::inv(pose_delta_result)*pose_delta_gt; // 真实点相对于预测点的位姿
      float r_err = rotationError(pose_error); //弧度
      float t_err = translationError(pose_error); // 这里是平移误差，位移差
      
      // compute speed
      float num_frames = (float)(last_frame-first_frame+1); //为啥+1？
      float speed = len/(0.1*num_frames); //单位m/s?
      
      // write to file
      err.push_back(errors(first_frame,r_err/len,t_err/len,len,speed)); //这里显示t_error没有单位，位移差/位移=相对平移误差，r_err是弧度/m，最后画图时转为角度/m
    }
  }
...
void savePathPlot (vector<Matrix> &poses_gt,vector<Matrix> &poses_result,string file_name) {
  ...
  for (int32_t i=0; i<poses_gt.size(); i+=step_size) //每隔3帧 记录gt和预测的x,z坐标,z轴指向前方
...
//从保存的error结构体记录中计算关于length和speed下的平均误差，用于作图
void saveErrorPlots(vector<errors> &seq_err,string plot_error_dir,char* prefix) {

  // file names tl：path lenth ts：speed
  ...
  for (int32_t i=0; i<num_lengths; i++) { //num_lengths=8
    ...
    for (vector<errors>::iterator it=seq_err.begin(); it!=seq_err.end(); it++) {
      if (fabs(it->len-lengths[i])<1.0) { //累加每个segment lenth 的error
        t_err += it->t_err;
        r_err += it->r_err;
        num++;
      }
  ...
  for (float speed=2; speed<25; speed+=2) {
    ...
    // for all errors do
    for (vector<errors>::iterator it=seq_err.begin(); it!=seq_err.end(); it++) {
      if (fabs(it->speed-speed)<2.0) { ////累加每个speed+-2 的error
        t_err += it->t_err;
        r_err += it->r_err;
        num++;
      }
...
//在某个序列error进行统计，记录平均值，我的补充
void saveseqStats (vector<errors> err,string dir,char* prefix) {
  ...
  char suffix[16];
  sprintf(suffix,"seqavgstats");
       
  // file name 例 seqavgstats_00.txt
  char file_name[1024];
  sprintf(file_name,"%s_%s.txt",suffix,prefix);

  // open file
  FILE *fp = fopen((dir + "/"+ file_name).c_str(),"w");
...
bool eval (string resultdir) {

  // ground truth and result directories
  string gt_dir         = "/home/dlr/kitti/dataset/poses";
  // string result_dir     = "/home/dlr/myimls/src/A-LOAM/" + result_sha; //myimls  results
  string result_dir     = resultdir;
  ...
  // for all sequences do  只对gt存在的序列00-10  原来是11-21
  for (int32_t i=0; i<11; i++) {
   
    // file name
    char file_name[256];
    sprintf(file_name,"%02d.txt",i); //file_name=i.txt
    
    // read ground truth and result poses vector中每个值都是4*4的矩阵，代表位姿
    vector<Matrix> poses_gt     = loadPoses(gt_dir + "/" + file_name);
    vector<Matrix> poses_result = loadPoses(result_dir + "/" + file_name);
   
    // plot status
    printf("Processing: %s, poses: %d/%d",file_name,int(poses_result.size()),int(poses_gt.size()));
    
    // check for errors
    if (poses_gt.size()==0 || poses_result.size()!=poses_gt.size()) {
      printf("ERROR: Couldn't read (all) poses of: %s", file_name);
//      return false;
      continue;
    }

    // compute sequence errors   这里向量中每个的error是结构体，填入向量容器
    vector<errors> seq_err = calcSequenceErrors(poses_gt,poses_result);
    saveSequenceErrors(seq_err,error_dir + "/" + file_name); //把当前序列每个error结构体中元素保存下来
    
    // add to total errors 插入到total_errors向量里
    ...
    if (i<=11) {  //15
      ...
      //计算每个序列的平均error -试着根据ORBSLAM
      saveseqStats(seq_err,result_dir,prefix );
    }
  }
  ...
  if (total_err.size()>0) {
    char prefix[16];
    sprintf(prefix,"avg");
    saveErrorPlots(total_err,plot_error_dir,prefix); //类似的，在所有序列的error记录进行作图
    plotErrorPlots(plot_error_dir,prefix);
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  if (argc!=2 && argc!=4) {
    cout << "Usage: ./eval_odometry resultdir" << endl;
    return 1;
  }

  // read arguments
  string resultdir = argv[1]; //注意这里是结果的全部路径

  
  cout << "Thank you for participating in our evaluation!" << endl;

  // run evaluation
  bool success = eval(resultdir);
