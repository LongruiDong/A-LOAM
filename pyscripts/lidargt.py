...
# 使用数据集中给定的相机-lidar的校准文件把gt中相机坐标系转为lidar坐标系！ calib_velo_to_cam.txt
...
for j in range(dataArray.shape[0]):
    dataArray[j] = np.matmul(Tvc, dataArray[j],)
...
f = open("/home/dlr/myimls/src/A-LOAM/result/04.txt", "w") #保存为新的gt位姿，初始为单位矩阵
for j in range(dataArray.shape[0]):
